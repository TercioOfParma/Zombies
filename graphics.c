...
void initGraphics()
{
	...
	noecho();
	cbreak();
	init_pair(PAIR_DEFAULT, COLOR_GREEN, COLOR_BLACK);//creates various colour schemes
...
{
	int x,y, character;
	x = entityPointer->position.xPos;//accesses the values of the position vector
...
void printEntityList(entity *entityList, int sizeOfList)
{
	...
	{
		if(entityList[mainLooper].dead != TRUE)//why would we print "dead" pits?
		{
